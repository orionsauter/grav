...
png(p(output.dir, "/timing.png"), width=600, height=600, res=100)
...
UpperLimits[,"band"]<-UpperLimits[,"first_bin"]/1800.0
...
png(p(output.dir, "/ul_allsky.png"), width=600, height=600, res=100)
...
png(p(output.dir, "/ul_lines.png"), width=600, height=600, res=100)
try({
...
png(p(output.dir, "/template_count.png"), width=600, height=600, res=100)
aggregate_plot(ylab="Template count")
dev.off()

png(p(output.dir, "/valid_count.png"), width=600, height=600, res=100)
aggregate_plot(quantity="SUM(valid_count)", ylab="Valid count")
dev.off()

png(p(output.dir, "/masked_count.png"), width=600, height=600, res=100)
aggregate_plot(quantity="LOG(SUM(masked_count)+1)", ylab="log10(Masked count+1)")
dev.off()

png(p(output.dir, "/masked_ratio.png"), width=600, height=600, res=100)
aggregate_plot(quantity="SUM(masked_count)/(SUM(masked_count)+SUM(valid_count))", ylab="Masked ratio")
dev.off()

png(p(output.dir, "/snr.png"), width=600, height=600, res=100)
...
Data[,"frequency"]<-Data[,"first_bin"]/1800.0

png(p(output.dir, "/snr_map.png"), width=600, height=600, res=100)
...
png(p(output.dir, "/gauss_test_max_m1_neg.png"), width=600, height=600, res=100)
aggregate_plot(quantity="MAX(IF(template_count=0, NULL, max_m1_neg))", ylab="max_m1_neg")
dev.off()

png(p(output.dir, "/gauss_test_min_m1_neg.png"), width=600, height=600, res=100)
aggregate_plot(quantity="MIN(IF(template_count=0, NULL, min_m1_neg))", ylab="min_m1_neg")
dev.off()

png(p(output.dir, "/gauss_test_max_m3_neg.png"), width=600, height=600, res=100)
aggregate_plot(quantity="MAX(IF(template_count=0, NULL, max_m3_neg))", ylab="max_m3_neg")
dev.off()

png(p(output.dir, "/gauss_test_min_m3_neg.png"), width=600, height=600, res=100)
aggregate_plot(quantity="MIN(IF(template_count=0, NULL, min_m3_neg))", ylab="min_m3_neg")
dev.off()

png(p(output.dir, "/gauss_test_max_m4.png"), width=600, height=600, res=100)
aggregate_plot(quantity="MAX(IF(template_count=0, NULL, log10(max_m4)))", ylab="log10(max_m4)")
dev.off()

png(p(output.dir, "/gauss_test_min_m4.png"), width=600, height=600, res=100)
...
data[,"ul"]<-data[,"ul"]*correction
plot((data$first_bin/1800), log10(data$ul), main=title, ylab="log10(h0)", xlab="Band", type="n")


FUL60Hz<- abs((data$first_bin/1800+1.25) %% 60)<2.50
points((data$first_bin[FUL60Hz]/1800), log10(data$ul[FUL60Hz]), col="cyan")
FUL<- !FUL60Hz

FNonGaussian<-(data[,"max_m1_neg"]>0.44 | data[,"min_m4"]<1.95)
F<-FUL & FNonGaussian
# F<- FUL & (band_info_full_ul$first_bin %in% SSmallKSValueBins)
points((data$first_bin[F]/1800), log10(data$ul[F]), col="blue", pch=23)
FUL<-FUL & !FNonGaussian

FDisturbedSpectrum<-data[,"max_m4"]>10
F<-FUL & FDisturbedSpectrum
# F<- FUL & (band_info_full_ul$first_bin %in% SSmallKSValueBins)
points((data$first_bin[F]/1800), log10(data$ul[F]), col="red", pch=23)
...
F<-FUL
points((data$first_bin[F]/1800), log10(data$ul[F]), col="green4", type="l")
}
...
plot((data$first_bin/1800), data$snr, main=title, ylab="SNR", xlab="Band", type="n")


FUL60Hz<- abs((data$first_bin/1800+1.25) %% 60)<2.50
points((data$first_bin[FUL60Hz]/1800), data$snr[FUL60Hz], col="cyan")
FUL<- !FUL60Hz

FNonGaussian<-(data[,"max_m1_neg"]>0.44 | data[,"min_m4"]<1.95) 
F<-FUL & FNonGaussian
# F<- FUL & (band_info_full_ul$first_bin %in% SSmallKSValueBins)
points((data$first_bin[F]/1800), data$snr[F], col="blue", pch=23)
FUL<-FUL & !FNonGaussian
# 
# F<- FUL & (band_info_full_ul$first_bin %in% SLargeKSValueBins)
# points((band_info_full_ul$first_bin[F]/1800), log10(band_info_full_ul$ul[F]), col="blue", pch=23)
...
F<-FUL & FHigh
# F<- FUL & (band_info_full_ul$first_bin %in% SSmallKSValueBins)
points((data$first_bin[F]/1800), data$snr[F], col="red", pch="+")
FUL<-FUL & !FHigh

F<-FUL
points((data$first_bin[F]/1800), data$snr[F], col="green4", type="l")
}
...
plot( (data$first_bin/1800), data$spindown, type="n", main=title, xlab="Band", ylab="Spindown")
...
for(i in 1:(length(breaks)-1)) {
	F<-Group==i
	F[is.na(F)]<-FALSE
	points( (data$first_bin/1800)[F], data$spindown[F], pch=19, col=Colors[[i]])
	}
...
png(p(output.dir, "/ul.png"), width=600, height=600, res=100)
ul_plot()
dev.off()

png(p(output.dir, "/circ.png"), width=600, height=600, correction=1.0, title="Circular polarization UL")
ul_plot(where.clause="skyband_name!='Lines' AND kind='circ'")
dev.off()

png(p(output.dir, "/circ_polar.png"), width=600, height=600, correction=1.0, title="Circular polarization UL")
...
png(p(output.dir, "/ul_midband.png"), width=600, height=600, res=100)
...
for(first_bin in high_bands) {
	...
	cat("Found", dim(coincidences)[1], "matches in band", first_bin/1800.0, "\n")
...
png(p(output.dir, "/band_info_snr_coincidences_f0fdot.png"), width=600, height=600, res=100)
print(xyplot(spindown~frequency, coincidences))
dev.off()

png(p(output.dir, "/band_info_snr_coincidences_sky.png"), width=600, height=600, res=100)
